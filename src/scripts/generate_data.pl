...
sub generate_grid {
	my( $p, $i, $j, @r );
	$p = rand_uniform_real( 0.01, 0.99 );
	for $i ( 1 .. $M ) {
		push @r, rand_bernoulli( $p );
	}
	return (\@r, $p);
}

sub rand_uniform_int {
	my( $min, $max ) = @_;
	return $min + (int rand ($max - $min));
}

sub rand_uniform_real {
	my( $min, $max ) = @_;
...
sub evolve_once {
	...
	for my $i ( 0 .. $N-1 ) {
		for my $j ( 0 .. $N-1 ) {
			...
			for my $di ( -1 .. 1 ) {
				for my $dj ( -1 .. 1 ) {
					$alive += get_cell( $i+$di, $j+$dj, $r );
				}
			}
			my $this = get_cell( $i, $j, $r );
			my $next = 0;
			$next = 1 if $alive == 3;
			$next = 1 if $alive == 4 and $this == 1;
			push @s, $next;
		}
	}
	return @s;
...
sub get_cell {
	my( $i, $j, $r ) = @_;
	return 0 unless 0 <= $i and $i < $N;
	return 0 unless 0 <= $j and $j < $N;
	return $r->[$i*$N + $j];
}
